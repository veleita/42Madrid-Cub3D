		...
		t_parameters *params)
{
	...
	ray->sprite_height = abs((int)(params->resolution_y / 
				ray->transform_y));
	// sprite_width seems to be equal to sprite_height
	ray->draw_start_y = params->resolution_y / 2 - 
		ray->sprite_height / 2;
	ray->draw_start_y = (ray->draw_start_y < 0) ? 0 : 
		ray->draw_start_y;
	ray->draw_end_y = params->resolution_y / 2 + 
		ray->sprite_height / 2;
	ray->draw_end_y = (ray->draw_end_y >= params->resolution_y) ?
		params->resolution_y - 1 : ray->draw_end_y;
	ray->draw_start_x = ray->sprite_screen_x - 
		ray->sprite_height / 2;
	ray->draw_start_x = (ray->draw_start_x < 0) ? 0 : 
		ray->draw_start_x;
	ray->draw_end_x = ray->sprite_screen_x + 
		ray->sprite_height / 2;
...
{
	ray->sprite_x = sprite[ray->sprite_order[it]]->pos_x -
		camera->pos_x;
	ray->sprite_y = sprite[ray->sprite_order[it]]->pos_y -
		camera->pos_y;
}
